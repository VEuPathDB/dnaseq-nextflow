package VEuPath::DbiDatabase;

use strict;
use VEuPath::DbiDbHandle;
use VEuPath::DbiTable;
use DBD::Oracle;
use VEuPath::PostgreSQL;
use Carp;
...
sub getDbPlatform{
	...
	if(!$self->{dbPlatform}){
		my $dsn=$self->getDSN();
		if($dsn=~/oracle/i){
			$self->{dbPlatform}=VEuPath::Oracle->new();
		}
		elsif($dsn=~/pg/i && $dsn!~/pgpp/i){
			$self->{dbPlatform}=VEuPath::PostgreSQL->new();
		}
		else{
...
sub makeNewHandle { 
  ...
  my $dbh = VEuPath::DbiDbHandle->new($self->getDSN(), $self->getLogin(),$self->getPassword(),
...
sub getTable {
  ...
  if ( ! $self->{'tables'}->{$className} ) {
	  ...
	  $self->{'tables'}->{$className} =
	      VEuPath::DbiTable->new($className,$self);
      }else{
...
sub addToDbCache {
  ...
  print STDERR "VEuPath::DbiDatabase->addToDbCache: adding ".$o->getClassName()." $pk\n" if $debug;
