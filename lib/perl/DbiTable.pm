...
sub getDatabase { 
  my ($self) = @_;
  if (!$self->{'database'}) {
    $self->{'database'} = VEuPath::DbiDatabase->getDefaultDatabase();
  }
...
sub sqlSelectAndGet {
  ...
  if (my $row = $sth->fetchrow_hashref('NAME_lc')) {
    if ($useInstanceClass) {
      ...
      return VEuPath::DbiRow->new($self->getClassName(), $row, $self->getDatabase()); ###CHECK THIS!!!!
    }
  } else {
    return undef;
  }
...
sub getNext {
  ...
  if (my $row = $self->{'sth'}->fetchrow_hashref('NAME_lc')) {
    if ($useInstanceClass) {
      ...
      return VEuPath::DbiRow->new($self->getTableName(), $row, $self->getDatabase()); ###CHECK THIS!!!!
    }
  } else {
    return undef;
  }
...
sub className2oracleName {
  my ($className) = @_;
  return VEuPath::DbiDatabase::className2oracleName($className);
}
