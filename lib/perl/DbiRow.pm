...
sub new {                       # constructor using DbiDatabase
  ...
  if (!$databaseObj) {
    $databaseObj = VEuPath::DbiDatabase->getDefaultDatabase();
  }
...
sub getRelations {              # returns list ref # this assume only one rel between tables!
    ...
    while (my $row = $sth->fetchrow_hashref('NAME_lc')) {
	if (!$object_type) {
	    $objects[$i] = VEuPath::DbiRow->new($foreign_table,$row,$self->getDatabase(),undef); 
	} else {
...
sub getDatabase {
  ...
  return VEuPath::DbiDatabase->getDefaultDatabase();
}
...
sub className2oracleName {
  my ($className) = @_;
  return VEuPath::DbiTable::className2oracleName($className);
}
