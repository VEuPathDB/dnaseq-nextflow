...
my $ploidy = 2;
my ($outputDir, $fpkm, $sampleName, $geneFootprintFile, $chrsForCalcFile);
...
&usage() unless ($outputDir && $fpkm && $sampleName && $geneFootprintFile);

die "File of gene footprints $geneFootprintFile not found\n" unless (-e $geneFootprintFile);

die "File of FPKM values $fpkm not found\n" unless (-e $fpkm); 

my $chrPloidies = &calculateChrPloidies($geneFootprintFile, $fpkm, $ploidy, $chrsForCalcFile);
...
sub calculateChrPloidies {
    my ($geneFootprintFile, $fpkmFile, $ploidy, $chrsForCalcFile) = @_;
    my $chrs = getChrsForCalcs($chrsForCalcFile);
...
sub usage {
    print STDERR "calculatePloidy --outputDir=s <dir to write output> --fpkmFile=s <full path to genes.fpkm_tracking file> --sampleName=s <sampleName used to name output files> --geneFootprints=s <gene footprint file> --chrsForCalcsFile=s <chrs for calcs file> [--ploidy=i <base ploidy, default 2>]\n";
    exit;
}
exit;
